...
However, there are so many variables so I need to filter them out before beginning the univariate analysis.
...
This result shows the number of NAs for each variable.
...
plot1 <-
  ggplot(aes(x=EstimatedReturn), 
       data=subset(df,
                  df$EstimatedReturn>.01 & 
                  df$EstimatedReturn<0.2)) +
...
plot2 <-
  ggplot(aes(x=EstimatedReturn), 
    ...
    geom_histogram(bins=50) +
    scale_x_log10() +
    ggtitle("EstimatedReturn Histogram (Scaled)")

# Plot histograms
grid.arrange(plot1, plot2, ncol = 1)
```

The graph is drawn after Estimated Return is filtered (more than 0.01 and less than 0.2). Similar to the previous variables, there is a peak at the right.
After applying the log10 scale, bimodal histogram can be seen.
...
These two histograms are very similar. 
Instead of using these 2 variables, I created a new variable 'CreditScoreRangeMid' that is the average of these 2.
...
There is an outlier at 10.0. So outlier is removed and plotted again.
...
plot2 <-
  ggplot(aes(x=StatedMonthlyIncome), 
    ...
    geom_histogram(bins = 100) +
    scale_y_log10() +
    ggtitle("StatedMonthlyIncome Histogram (scale y axis log10)")
...
```{r echo=FALSE, MonthlyIncome_continued_2}
...
After applying log10 to y axis, the graph now looks better.
...
df <- df %>% 
  mutate(ratio_monthly_loan_payment = 
           ifelse(StatedMonthlyIncome==0,
                  0,
...
df$ratio_monthly_loan_payment.bucket <- 
  cut(df$ratio_monthly_loan_payment, 
      breaks=c(-0.01,.5,12571))
...
```{r echo=FALSE, ratio_histogram_filtered}
# Plot the histogram for ratio_monthly_loan_payment less than 0.5
ggplot(aes(x=ratio_monthly_loan_payment), 
       data=subset(df,
...
# Plot a scatter plot between CreditScoreRangeMid and BorrowerRates 
# filtered on CreditScoreRangeMid greater than 300
plot2 <-
...
tmp <- subset(df,CreditScoreRangeMid>400)
cor.test(tmp$BorrowerRate,
         tmp$CreditScoreRangeMid, 
         type='pearson')
```
...
```{r echo=FALSE, summary_status_income , fig.width=11, fig.height=11}
...
```{r echo=FALSE, summary_status_rate , fig.width=11, fig.height=11}
...
f <- lm(LoanOriginalAmount ~ CreditScoreRangeMid,
        data=subset(df,CreditScoreRangeMid>350))
summary(f)
```
...
cor.test(
  filter_monthlyIncome$StatedMonthlyIncome, 
  filter_monthlyIncome$CreditScoreRangeMid)
...
tmp <- subset(df,StatedMonthlyIncome>10)
...
ggplot(aes(
          x=CreditScoreRangeMid,
          y=ratio_monthly_loan_payment,
          color=ProsperScore
          ), 
       data = subset(df,
                     !is.na(ProsperScore) &
...
df$ProsperScore.bucket <- cut(df$ProsperScore.numeric, 
...
ggplot(aes(
  ...
  ggtitle("Facet Wrap by Borrower Rate")
```
...
```{r echo=FALSE, LoanAmt_LoanPmt_ProsperScore_Rate}
...
df$StatedMonthlyIncome.bucket <- 
  cut(df$StatedMonthlyIncome,
      breaks=c(
        0,1000,2000,3000,4000,5000,6000,7000,8000,1.75e+06
        )
      )
...
ggplot(aes(
        x=ProsperScore,
        y=BorrowerRate,
        color=StatedMonthlyIncome.bucket
        ), 
       data=df) +
...
ggplot(
  ...
  scale_colour_gradient2(low="white", high=muted("blue")) +
  theme_dark() +
  labs(colour = "ProsperScore") +
  ggtitle("CreditScoreRangeMid vs. Monthly Income colored by ProsperScore")
```

### Description Three

Through this graph, the linear model can be constructed using ProsperScore, CreditScore and Monthly Income.
This graph shows the that people with good ProsperScore and CreditScore have large monthly income and people with bad ProsperScore and CreditScore have low monthly income. Note the smooth line keeps increasing as the Credit Score increases.
