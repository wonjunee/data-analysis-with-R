...
ggplot(aes(x=CreditScoreRangeMid,y=ratio_monthly_loan_payment,color=ProsperScore), 
  ...
  labs("Facet Wrap by Borrower APR")
```
...
```{r}
ggplot(aes(y=ratio_monthly_loan_payment,x=EstimatedReturn,color=ProsperScore), data=subset(df,ratio_monthly_loan_payment<=.2 & EstimatedReturn<0.2 & EstimatedReturn>0.0)) +
  geom_jitter(alpha=.05, size=2)
...
```{r}
ggplot(aes(x=ProsperScore,y=BorrowerRate,color=LoanStatus), 
       data=subset(df,
